# What EntityProperties.Weight really reads at runtime. Headless dedicated server ONLY -
# no client, nothing opens on the desktop. Vanilla declares creature weight per VARIANT
# (bear-sun 57 ... bear-polar 705); the size-tiered bleed ladder is only viable if that
# survives to the runtime property instead of collapsing per species or falling back to
# the 25kg engine default. Dumps the resolved numbers so bracket lines get drawn against
# reality.
...
$port   = 42493   # distinct from live 42420 and every other harness port
...
$run   = Join-Path $scratch ("weightprobe\" + [Guid]::NewGuid().ToString("N").Substring(0,8))
...
$env:TASSHUNTING_WEIGHTPROBE = "1"
$sp = Start-Process $srvExe -ArgumentList @("--dataPath",$sdata) -PassThru -WindowStyle Hidden
$env:TASSHUNTING_WEIGHTPROBE = $null

$complete = $false
for ($i=0; $i -lt 50; $i++) {
    Start-Sleep -Seconds 3
    if ((Test-Path $slog) -and (Select-String -Path $slog -Pattern "WEIGHTPROBE COMPLETE" -Quiet)) { $complete = $true; break }
}
try { Stop-Process -Id $sp.Id -Force -ErrorAction SilentlyContinue } catch {}

Write-Host "===== RESULT: complete=$complete ====="
if (Test-Path $slog) {
    Select-String -Path $slog -Pattern "\[weightprobe\]|\[Error\]|Exception" | ForEach-Object { $_.Line } | Select-Object -First 40
}
if (-not $complete) { Write-Host "WEIGHTPROBE FAILED TO COMPLETE - inspect $slog" }
